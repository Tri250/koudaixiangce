...
const panelGroups: Array<Array<PanelOptions>> = [
  ...
  [
    ...
    { id: Panel.Portrait, icon: User, title: 'editor.switcher.tooltips.portrait' },
...
export default function RightPanelSwitcher({
  ...
  return (
    <div className={isHorizontal ? 'flex items-center overflow-x-auto p-1 gap-1 min-w-0' : 'flex flex-col p-1 gap-1 h-full min-w-[300px] max-w-[420px] mx-auto'}>
        ...
        <div key={groupIndex} className={isHorizontal ? 'flex items-center gap-1' : 'flex flex-col gap-1'}>
          ...
          {group.map(({ id, icon: Icon, title }) => (
            <button
              ...
              data-tooltip={t(title as unknown)}
            >
              {activePanel === id && (
                <motion.div
