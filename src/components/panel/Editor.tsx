...
export default function Editor({ onBackToLibrary, onContextMenu, transformWrapperRef }: EditorProps) {
  ...
  const prevCropParams = useRef<HTMLElement>(null);
  ...
  const pendingOverlayRequestRef = useRef<HTMLElement>(null);
  ...
  const handleDisplaySizeChange = useCallback(
    (size: { width: number; height: number }) => {
      ...
      if (size.scale) {
        ...
        setEditor({ baseRenderSize: newSize as unknown });
      }
    },
    [setEditor],
  );
  ...
  const processOverlayQueue = useCallback(async () => {
    ...
    try {
      ...
      const stripSubMasks = (subMasks: unknown[]) => {
  ...
  const overlayTriggerHash = useMemo(() => {
    ...
    geometryKeys.forEach((k) => {
      geometry[k] = (adjustments as unknown)[k];
    });
