...
export function ContextMenuProvider({ children }: ContextMenuProviderProps) {
  ...
  // --- Long-press support for touch devices (C5) ---
  // On Android, right-click is unavailable. We detect a long-press (500ms
  // without significant finger movement) and dispatch a synthetic
  // `contextmenu` event at the touch coordinates. React's event delegation
  // picks it up and the existing `onContextMenu` handlers fire naturally,
  // so no consumer component needs to change.
  useEffect(() => {
    ...
    const handleTouchStart = (e: TouchEvent) => {
      ...
      touchTimer = setTimeout(() => {
        ...
        // Dispatch a synthetic contextmenu event. React 17+ delegates events
        // to the root container, so a native dispatch bubbling from the target
        // will trigger any onContextMenu handler attached via React props.
