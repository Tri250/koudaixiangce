...
export default function GlobalTooltip() {
  ...
  useEffect(() => {
    ...
    // --- Touch / long-press support (C4) ---
    // On touch devices, `mouseover`/`mouseout` fire unreliably (only on tap,
    // and immediately followed by `mousedown` which dismisses the tooltip).
    // We add a dedicated long-press path: touchstart starts a timer, and if
    // the finger stays put for TOOLTIP_DELAY ms, the tooltip shows. Any
    // movement or lift cancels it.
    let lastTouchAt = 0;
    ...
    const handleTouchStart = (e: TouchEvent) => {
      ...
      timerRef.current = setTimeout(() => {
        ...
        setLeftOverride(null);
        setTooltip(data);
        // Don't watchTarget on touch — the element won't move under the finger.
      }, TOOLTIP_DELAY);
    };

    const handleTouchMove = () => {
      // Any finger movement cancels the long-press (likely scrolling).
      clearTimer();
      if (tooltip) hide();
    };
    ...
    const handleTouchEnd = () => {
      clearTimer();
      // Keep the tooltip visible briefly so the user can read it after lifting
      // the finger, then auto-hide after a short delay.
      if (tooltip) {
        timerRef.current = setTimeout(() => {
          timerRef.current = null;
          hide();
    ...
    const dismiss = (e: MouseEvent) => {
      // Synthetic mouse events from touch (detail === 0) should not dismiss
      // a tooltip that was triggered by long-press.
      if (e.detail === 0 && Date.now() - lastTouchAt < 1500) return;
      hide();
    };
