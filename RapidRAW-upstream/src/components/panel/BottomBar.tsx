...
const StarRating = ({ rating, onRate, disabled, isAndroid }: StarRatingProps) => {
...
export default function BottomBar({
  ...
  // Touch handlers for Android — mirror zoom_in_step / zoom_out_step / zoom_fit / zoom_100 keyboard shortcuts.
  const handleTouchZoomIn = () => {
    onZoomChange(Math.min(currentOriginalPercent + 0.1, 2.0));
  };

  const handleTouchZoomOut = () => {
    onZoomChange(Math.max(currentOriginalPercent - 0.1, 0.1));
  };

  const handleTouchZoomFit = () => {
    onZoomChange(0, true);
  };

  const handleTouchZoom100 = () => {
    onZoomChange(1.0);
  };

  // Touch handlers for Android — mirror rotate_left / rotate_right keyboard shortcuts.
  const handleTouchRotateLeft = () => {
    handleRotate(-90);
  };

  const handleTouchRotateRight = () => {
    handleRotate(90);
  };

  const touchActionDisabled = !selectedImage;
          ...
          <div className="flex items-center gap-1">
            ...
            {showFilmstrip && (
              <>
                ...
                >
                  {isFilmstripVisible ? <ChevronDown size={20} /> : <ChevronUp size={20} />}
                </button>
              </>
            )}
          </div>
