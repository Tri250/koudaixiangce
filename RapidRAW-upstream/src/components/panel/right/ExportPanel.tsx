...
export default function ExportPanel({
  ...
  const handleShare = async () => {
    ...
    try {
      await invoke(Invokes.ShareImageOnAndroid, { filePath: lastExportedFileName });
    } catch (err) {
      toast.error(t('errors.shareFailed', { message: String(err) }));
    }
  };
  ...
  return (
    <div className={onClose ? 'h-full bg-bg-secondary rounded-lg flex flex-col' : 'flex flex-col h-full'}>
      ...
      <div className="p-4 border-t border-surface shrink-0 space-y-2">
        ...
        {isAndroid && status === Status.Success && lastExportedFileName && (
          <Button
            className="rounded-md h-11 w-full flex items-center justify-center bg-surface hover:bg-card-active text-text-primary"
            onClick={handleShare}
            title={t('export.shareTooltip')}
            size="lg"
          >
            <Share2 size={18} className="mr-2" /> {t('export.share')}
          </Button>
        )}
      </div>
    </div>
