...
import { OPTION_SEPARATOR, Invokes } from '../../ui/AppProperties';
...
export default function AIPanel() {
  ...
  // AI 追色 (Color Match) state — Qwen-VL powered color grading transfer.
  const [referencePath, setReferencePath] = useState<string | null>(null);
  const [isColorMatching, setIsColorMatching] = useState(false);
  const [colorMatchDescription, setColorMatchDescription] = useState<string | null>(null);
  ...
  // ---- AI 追色 (Color Match) handlers ----

  const getReferenceFileName = useCallback((path: string) => {
  ...
  const handleSelectReference = useCallback(async () => {
    try {
      ...
      if (typeof selected === 'string') {
        ...
        setColorMatchDescription(null);
      }
    } catch (err) {
      console.error('Failed to open reference image dialog:', err);
    }
  }, [t]);

  const handleApplyColorMatch = useCallback(async () => {
    if (!selectedImage?.path) {
      toast.error(t('editor.ai.colorMatch.noReference'));
      return;
    }
    if (!referencePath) {
      toast.error(t('editor.ai.colorMatch.noReference'));
    ...
    const vlmApiKey = appSettings?.vlmApiKey;
    if (!vlmApiKey || !vlmApiKey.trim()) {
      toast.error(t('editor.ai.colorMatch.noApiKey'));
      return;
    }
    ...
    setColorMatchDescription(null);
    try {
      const result = await invoke(Invokes.AiColorMatch, {
        sourcePath: selectedImage.path,
        referencePath: referencePath,
      });

      const cm = result as any;
      const description: string = cm.description || '';
      setColorMatchDescription(description);

      // Merge the VLM-produced adjustments into the current image. The VLM
      // response keys (exposure, temperature, ... colorGrading) match the
      // Adjustments interface directly; colorGrading is merged field-by-field
      // so we don't clobber balance / blending / global.
      const cg = cm.colorGrading || {};
      setAdjustments((prev: Adjustments) => ({
        ...prev,
        exposure: cm.exposure ?? prev.exposure,
        temperature: cm.temperature ?? prev.temperature,
        tint: cm.tint ?? prev.tint,
        contrast: cm.contrast ?? prev.contrast,
        highlights: cm.highlights ?? prev.highlights,
        shadows: cm.shadows ?? prev.shadows,
        whites: cm.whites ?? prev.whites,
        blacks: cm.blacks ?? prev.blacks,
        vibrance: cm.vibrance ?? prev.vibrance,
        saturation: cm.saturation ?? prev.saturation,
        clarity: cm.clarity ?? prev.clarity,
        sharpness: cm.sharpness ?? prev.sharpness,
        toneMapper: cm.toneMapper ?? prev.toneMapper,
        colorGrading: {
          ...prev.colorGrading,
          ...(cg.shadows ? { shadows: cg.shadows } : {}),
          ...(cg.midtones ? { midtones: cg.midtones } : {}),
          ...(cg.highlights ? { highlights: cg.highlights } : {}),
        },
      }));

      toast.success(t('editor.ai.colorMatch.success', { description: description || '' }));
    } catch (err) {
      toast.error(String(err));
    } finally {
      setIsColorMatching(false);
    }
  }, [selectedImage, referencePath, appSettings, setAdjustments, t]);
    ...
    >
      <div className="flex flex-col h-full select-none overflow-hidden" onContextMenu={handlePanelContextMenu}>
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden flex flex-col min-h-0 p-4">
          ...
          {/* AI 追色 (Color Match) — Qwen-VL powered color grading transfer.
              The user picks a reference image; the VLM analyses its tonal
              style and returns a JSON contract of adjustments that are merged
              into the current image. */}
          {selectedImage && (
            <div
              className="shrink-0 p-4 rounded-lg bg-surface"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex items-center gap-2 mb-1">
                <Palette size={18} className="text-text-secondary" />
                <Text variant={TextVariants.heading}>{t('editor.ai.colorMatch.title')}</Text>
              </div>
              <Text variant={TextVariants.small} color={TextColors.secondary} className="block mb-3">
                {t('editor.ai.colorMatch.description')}
              </Text>

              <div className="space-y-3">
                <button
                  className="w-full p-3 rounded-md bg-bg-tertiary hover:bg-card-active transition-colors flex items-center gap-2 text-left disabled:opacity-50 disabled:cursor-not-allowed"
                  onClick={handleSelectReference}
                  disabled={isColorMatching}
                  data-tooltip={t('editor.ai.colorMatch.selectReference')}
                >
                  <FolderOpen size={18} className="shrink-0 text-text-secondary" />
                  <Text
                    as="div"
                  ...
                  >
                    {referencePath
                      ? t('editor.ai.colorMatch.referenceSelected', {
                          name: getReferenceFileName(referencePath),
                        })
                      : t('editor.ai.colorMatch.selectReference')}
                  </Text>
                </button>

                <Button
                  className="w-full"
                  disabled={isColorMatching || !referencePath}
                ...
                >
                  ...
                  <span className="ml-2">
                    {isColorMatching
                      ? t('editor.ai.colorMatch.applying')
                      : t('editor.ai.colorMatch.apply')}
                  </span>
                </Button>
                ...
                {colorMatchDescription && (
                  <Text
                    ...
                    variant={TextVariants.small}
                    color={TextColors.secondary}
                    className="block p-2 bg-bg-tertiary rounded-md italic"
                  >
                    {colorMatchDescription}
                  </Text>
                )}
