...
const CloudDashboard = ({ cloudBaseUrl }: { cloudBaseUrl?: string }) => {
  const { user } = useUser();
  ...
  // M11: resolve cloud API base URL. User-set value wins; otherwise fall back to
  // the production default. The Rust backend additionally honors the
  // RAPIDRAW_CLOUD_BASE_URL env var for inpainting calls.
  const resolvedBaseUrl = (cloudBaseUrl && cloudBaseUrl.trim().length > 0
    ? cloudBaseUrl.trim()
    : 'https://getrapidraw.com/api'
  ).replace(/\/$/, '');
  ...
  useEffect(() => {
    const fetchUsage = async () => {
      try {
        ...
        if (!token) return;
        const res = await fetch(`${resolvedBaseUrl}/usage`, {
...
export default function SettingsPanel({
  ...
  const [cloudBaseUrl, setCloudBaseUrl] = useState<string>(appSettings?.cloudBaseUrl || '');
  const [vlmApiKey, setVlmApiKey] = useState<string>(appSettings?.vlmApiKey || '');
  const [vlmEndpoint, setVlmEndpoint] = useState<string>(appSettings?.vlmEndpoint || '');
  const [vlmModel, setVlmModel] = useState<string>(appSettings?.vlmModel || '');
  ...
  useEffect(() => {
    ...
    if ((appSettings?.cloudBaseUrl || '') !== cloudBaseUrl) {
      setCloudBaseUrl(appSettings?.cloudBaseUrl || '');
    }
    if ((appSettings?.vlmApiKey || '') !== vlmApiKey) {
      setVlmApiKey(appSettings?.vlmApiKey || '');
    }
    if ((appSettings?.vlmEndpoint || '') !== vlmEndpoint) {
      setVlmEndpoint(appSettings?.vlmEndpoint || '');
    }
    if ((appSettings?.vlmModel || '') !== vlmModel) {
      setVlmModel(appSettings?.vlmModel || '');
    }
                    ...
                    <AnimatePresence mode="wait">
                      ...
                      {aiProvider === 'cloud' && (
                        ...
                        >
                          ...
                          {/* M11: advanced override for the cloud API base URL.
                              Useful for self-hosters / enterprise / CI redirection.
                              Empty value restores the production default. */}
                          <div className="mt-6 pt-6 border-t border-border-color">
                            <SettingItem
                              label={t('settings.processing.ai.cloud.advanced.baseUrl.label')}
                              description={t('settings.processing.ai.cloud.advanced.baseUrl.description')}
                            >
                              <Input
                                className="w-full"
                                id="cloud-base-url"
                                onBlur={() =>
                                  onSettingsChange({ ...appSettings, cloudBaseUrl: cloudBaseUrl.trim() })
                                }
                                onChange={(e: any) => setCloudBaseUrl(e.target.value)}
                                onKeyDown={(e: any) => e.stopPropagation()}
                                placeholder="https://getrapidraw.com/api"
                                type="text"
                                value={cloudBaseUrl}
                                bgClassName="bg-bg-primary"
                              />
                          ...
                          {/* VLM (Qwen-VL) configuration for AI Color Match.
                              API key is required; endpoint/model fall back to
                              DashScope defaults when left blank. */}
                          <div className="mt-6 pt-6 border-t border-border-color space-y-6">
                            <SettingItem
                              label={t('settings.processing.ai.cloud.advanced.vlm.apiKey.label')}
                              description={t('settings.processing.ai.cloud.advanced.vlm.apiKey.description')}
                            >
                              <Input
                                className="w-full"
                                id="vlm-api-key"
                                onBlur={() =>
                                  onSettingsChange({ ...appSettings, vlmApiKey: vlmApiKey.trim() })
                                }
                                onChange={(e: any) => setVlmApiKey(e.target.value)}
                                onKeyDown={(e: any) => e.stopPropagation()}
                                placeholder="sk-..."
                                type="password"
                                value={vlmApiKey}
                            ...
                            <SettingItem
                              label={t('settings.processing.ai.cloud.advanced.vlm.endpoint.label')}
                              description={t('settings.processing.ai.cloud.advanced.vlm.endpoint.description')}
                            >
                              <Input
                                className="w-full"
                                id="vlm-endpoint"
                                onBlur={() =>
                                  onSettingsChange({ ...appSettings, vlmEndpoint: vlmEndpoint.trim() })
                                }
                                onChange={(e: any) => setVlmEndpoint(e.target.value)}
                                onKeyDown={(e: any) => e.stopPropagation()}
                            ...
                            <SettingItem
                              label={t('settings.processing.ai.cloud.advanced.vlm.model.label')}
                              description={t('settings.processing.ai.cloud.advanced.vlm.model.description')}
                            >
                              <Input
                                className="w-full"
                                id="vlm-model"
                                onBlur={() =>
                                  onSettingsChange({ ...appSettings, vlmModel: vlmModel.trim() })
                                }
                                onChange={(e: any) => setVlmModel(e.target.value)}
                                onKeyDown={(e: any) => e.stopPropagation()}
