...
export const debouncedSave = debounce((path: string, adjustmentsToSave: Adjustments) => {
  invoke(Invokes.SaveMetadataAndUpdateThumbnail, { path, adjustments: adjustmentsToSave }).catch((err) => {
    ...
    toast.error(i18n.t('errors.saveChangesFailed', { message: String(err) }));
  });
}, 300);
...
export function useEditorActions() {
  ...
  const handleAutoAdjustments = useCallback(async () => {
    ...
    try {
      ...
      toast.error(t('errors.applyAutoAdjustmentsFailed', { message: String(err) }));
    }
  }, [setAdjustments, t]);
  ...
  const handleLutSelect = useCallback(
    async (path: string) => {
      ...
      try {
        ...
        toast.error(t('errors.loadLutFailed', { message: String(err) }));
      }
    },
    [setAdjustments, t],
  );
  ...
  const handleResetAdjustments = useCallback(
    (paths?: string[]) => {
      ...
      invoke(Invokes.ResetAdjustmentsForPaths, { paths: pathsToReset })
        ...
        .catch((err) => toast.error(t('errors.resetAdjustmentsFailed', { message: String(err) })));
    },
    [setEditor, t],
  );
    ...
    if (selectedImage) {
      ...
      if (pathToCopyFrom) {
        try {
          ...
          toast.error(t('errors.loadMetadataForCopyFailed', { message: String(err) }));
          return;
        }
      }
    }
  ...
  const handlePasteAdjustments = useCallback(
    (paths?: string[]) => {
      ...
      invoke(Invokes.ApplyAdjustmentsToPaths, { paths: pathsToUpdate, adjustments: adjustmentsToApply })
        ...
        .catch((err) => toast.error(t('errors.pasteAdjustmentsFailed', { message: String(err) })));
