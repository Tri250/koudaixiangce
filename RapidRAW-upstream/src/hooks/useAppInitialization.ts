...
import { globalImageCache, computeAdaptiveImageCacheSize } from '../utils/ImageLRUCache';
import {
  FilterCriteria,
  Invokes,
  LibraryViewMode,
...
const getDefaultLanguage = (i18nInstance: any): string => {
  ...
  // Direct match (e.g., "zh-CN" → "zh-CN")
  if (supportedLanguages.includes(browserLang)) return browserLang;

  // Handle BCP 47 script subtags (e.g., "zh-Hans-CN" → "zh-CN", "zh-Hant-TW" → "zh-TW")
  // Android 14+ may return script-tagged locales. Strip the script subtag and retry.
  const parts = browserLang.split('-');
  if (parts.length >= 3) {
    // Format: language-Script-Region → remove the Script part, rejoin as language-Region
    const withoutScript = [parts[0], parts[2]].join('-');
    if (supportedLanguages.includes(withoutScript)) return withoutScript;
  }

  // Short-language match (e.g., "en-US" → "en")
  const shortLang = parts[0].toLowerCase();
  if (supportedLanguages.includes(shortLang)) return shortLang;

  // Map common short codes to our full locale codes
  if (shortLang === 'zh') return supportedLanguages.includes('zh-CN') ? 'zh-CN' : fallbackLang;
...
export const useAppInitialization = ({
  ...
  // M4: tune the frontend image preview LRU to the actual platform / memory
  // budget. On Android phones we cap the cache aggressively (4-6 entries) to
  // avoid OOM kills; desktop keeps the historical 20-entry default. Re-runs
  // when the platform has been resolved by initPlatform().
  useEffect(() => {
    if (!osPlatform) return;
    const target = computeAdaptiveImageCacheSize(osPlatform);
    if (target !== globalImageCache.capacity) {
      globalImageCache.setMaxSize(target);
    }
  }, [osPlatform]);
