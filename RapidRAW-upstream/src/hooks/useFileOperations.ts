  ...
  const executeDelete = useCallback(
    async (pathsToDelete: Array<string>, options = { includeAssociated: false }) => {
      ...
      try {
        ...
        toast.error(t('errors.deleteFilesFailed', { message: String(err) }));
      }
    },
    [refreshImageList, handleBackToLibrary, sortedImageList, handleImageSelect, t],
  );
    ...
    async (folderName: string) => {
      ...
      if (folderName && folderName.trim() !== '' && folderActionTarget) {
        try {
          ...
          toast.error(t('errors.createFolderFailed', { message: String(err) }));
        }
      }
    },
    [refreshAllFolderTrees, t],
  );
    ...
    async (newName: string) => {
      ...
      if (newName && newName.trim() !== '' && folderActionTarget) {
        try {
          ...
          toast.error(t('errors.renameFolderFailed', { message: String(err) }));
        }
      }
    },
    [refreshAllFolderTrees, t],
    ...
    async (nameTemplate: string) => {
      ...
      if (renameTargetPaths.length > 0 && nameTemplate) {
        try {
          ...
          toast.error(t('errors.renameFilesFailed', { message: String(err) }));
        }
      }
      setUI({ renameTargetPaths: [] });
    },
    [refreshImageList, handleImageSelect, handleBackToLibrary, t],
  );
      ...
      try {
        ...
        if (Array.isArray(selected) && selected.length > 0) {
          ...
          if (invalidExtensions.size > 0) {
            ...
            toast.error(t('errors.unsupportedFileFormats', { formats: extList }));
            return;
          }
  ...
  const handlePasteFiles = useCallback(
    async (mode = 'copy') => {
      ...
      try {
        ...
        toast.error(
          mode === 'copy'
            ? t('errors.copyFilesFailed', { message: String(err) })
            : t('errors.moveFilesFailed', { message: String(err) }),
        );
      }
    },
    [refreshImageList, refreshAllFolderTrees, t],
  );
