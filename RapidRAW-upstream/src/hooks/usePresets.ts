...
export function usePresets(currentAdjustments: Adjustments) {
  ...
  const loadPresets = useCallback(async () => {
    setIsLoading(true);
    try {
      // Seed the curated "国内场景" preset folder on first launch / if missing.
      // Idempotent on the Rust side (checks folder by name), so calling it
      // unconditionally on every load is cheap and keeps users who upgrade
      // from older versions without losing their existing presets.
      try {
        await invoke(Invokes.SeedDefaultPresets);
      } catch (seedErr) {
        // Seeding is best-effort — never block preset loading on it.
        console.warn('Failed to seed default presets:', seedErr);
      }
