...
export function useAppNavigation({ clearThumbnailQueue, refs }: AppNavigationProps) {
  ...
  const handleSelectSubfolder = useCallback(
    async (
      ...
      try {
        ...
        toast.error(t('errors.loadImagesFromFolder'));
      } finally {
        useLibraryStore.getState().setLibrary({ isViewLoading: false });
      }
    },
    [clearThumbnailQueue, refs, t],
  );
  ...
  const handleSelectAlbum = useCallback(
    async (albumId: string, albumName: string, imagePaths: string[], preserveEditor = false) => {
      ...
      try {
        ...
        toast.error(t('errors.loadAlbumFailed', { message: String(err) }));
      } finally {
        setLibrary({ isViewLoading: false });
      }
    },
  ...
  const handleOpenFolder = async () => {
    ...
    try {
      let selectedPath = '';
      if (isAndroid) {
        // H1: Use the Storage Access Framework directory picker so users can
        // select external SD cards, gallery directories, or NAS mount points —
        // not just the private internal library root. The Kotlin
        // MainActivity.onActivityResult bridges the SAF result back via
        // window.__RapidRAWSAFPick(uri|null), which forwards it to
        // resolve_android_saf_pick to unblock pick_android_directory.
        let safBridgeUri: string | null | undefined = undefined;
        (window as any).__RapidRAWSAFPick = async (uri: string | null) => {
          safBridgeUri = uri;
          delete (window as any).__RapidRAWSAFPick;
          try {
            await invoke(Invokes.ResolveAndroidSafPick, { uri });
          } catch {
            // The pending request may already be cleared (e.g. timeout); ignore.
          }
        };

        try {
          selectedPath = await invoke<string>(Invokes.PickAndroidDirectory);
        } catch (err) {
          delete (window as any).__RapidRAWSAFPick;
          if (safBridgeUri === null) {
            // User cancelled the SAF picker — fall back to internal root.
            toast.info(t('errors.androidSafCancelled'));
          } else {
            console.error('Android SAF directory picker failed:', err);
            toast.error(t('errors.androidSafFailed', { message: String(err) }));
          }
      ...
      if (selectedPath) {
        if (!rootPaths.includes(selectedPath)) {
          ...
          try {
            ...
            toast.error(t('errors.loadFolderTreeFailed', { message: String(e) }));
          } finally {
      ...
      toast.error(isAndroid ? t('errors.openLibraryFailed') : t('errors.openFolderDialogFailed'));
    }
  };
  ...
  const handleContinueSession = () => {
    ...
    restore().catch((err) => {
      ...
      toast.error(t('errors.restoreSessionFailed'));
      handleGoHome();
