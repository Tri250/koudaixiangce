...
pub struct AppSettings {
    ...
    /// Optional override for the RapidRAW cloud API base URL.
    /// When unset, falls back to the `RAPIDRAW_CLOUD_BASE_URL` env var,
    /// then to the production default `https://getrapidraw.com/api`.
    /// Lets self-hosters / enterprise / CI redirect or disable cloud calls
    /// without rebuilding the app.
    #[serde(default)]
    pub cloud_base_url: Option<String>,
    /// Optional API key for vision-language-model (VLM) based features like
    /// AI color matching. When unset, those features surface a clear error.
    #[serde(default)]
    pub vlm_api_key: Option<String>,
    /// Optional override for the VLM endpoint. Defaults to DashScope
    /// (https://dashscope.aliyuncs.com/compatible-mode/v1) when unset, which
    /// serves Qwen-VL-Max. Self-hosters can point this at a local VLM.
    #[serde(default)]
    pub vlm_endpoint: Option<String>,
    /// Optional model name override. Defaults to "qwen-vl-max-latest".
    #[serde(default)]
