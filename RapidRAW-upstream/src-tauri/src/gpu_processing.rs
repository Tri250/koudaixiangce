...
#[repr(C)]
#[derive(Debug, Copy, Clone, bytemuck::Pod, bytemuck::Zeroable)]
pub struct SkinDetectionParams {
    pub threshold_low: f32,
    pub threshold_high: f32,
    pub weight_ycgcr: f32,
    pub weight_ycbcr: f32,
    pub weight_rgb: f32,
    pub weight_hsv: f32,
    pub is_linear_rgb: u32,
    pub _pad0: u32,
}

impl Default for SkinDetectionParams {
    fn default() -> Self {
        Self {
            threshold_low: 0.3,
            threshold_high: 0.5,
...
pub struct GpuProcessor {
    ...
    skin_detection_bgl: wgpu::BindGroupLayout,
    skin_detection_pipeline: wgpu::ComputePipeline,
...
impl GpuProcessor {
    pub fn new(context: GpuContext, max_width: u32, max_height: u32) -> Result<Self, String> {
        ...
        let skin_detection_shader_module = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Skin Detection Shader"),
            source: wgpu::ShaderSource::Wgsl(include_str!("shaders/skin_detection.wgsl").into()),
        });

        let skin_detection_bgl = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("Skin Detection BGL"),
            entries: &[
                wgpu::BindGroupLayoutEntry {
                    binding: 0,
                    visibility: wgpu::ShaderStages::COMPUTE,
        ...
        let skin_detection_pipeline_layout =
            device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                label: Some("Skin Detection Pipeline Layout"),
                bind_group_layouts: &[Some(&skin_detection_bgl)],
                immediate_size: 0,
            });

        let skin_detection_pipeline =
            device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
                label: Some("Skin Detection Pipeline"),
                layout: Some(&skin_detection_pipeline_layout),
                module: &skin_detection_shader_module,
                entry_point: Some("detect_skin"),
                compilation_options: Default::default(),
                cache: None,
            });
    ...
    /// Runs the 4-method weighted skin-detection compute shader.
    ///
    /// `source_view` is an RGBA texture view (sRGB-encoded or linear, per
    /// `params.is_linear_rgb`). `dest_view` is an `R8Unorm` write-only storage
    /// view. The shader writes skin confidence [0,1] to the R channel.
    /// Dispatches `(width+15)/16 x (height+15)/16` workgroups of 16x16.
    pub fn detect_skin(
        &self,
        ...
        let uniform_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("Skin Detection Params Buffer"),
            contents: bytemuck::bytes_of(&params),
            usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
        });

        let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("Skin Detection BG"),
            layout: &self.skin_detection_bgl,
            entries: &[
                wgpu::BindGroupEntry {
                    binding: 0,
        ...
        let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
            label: Some("Skin Detection Encoder"),
        });

        {
            let mut cpass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
                label: Some("Skin Detection Pass"),
                timestamp_writes: None,
            });
            cpass.set_pipeline(&self.skin_detection_pipeline);
