...
fn analyze_image(
    ...
    // M14: ANALYSIS_DIM caps the longest side of the image used for culling
    // analysis. Downsampling is deliberate, not a bug:
    //
    //   - Performance: a 50 MP RAW decoded to RGB is ~200 MB; running a
    //     Laplacian variance over the full buffer for hundreds of images in a
    //     culling pass would be prohibitively slow.
    //   - Relative ranking: culling compares sharpness ACROSS images of the
    //     same shoot. As long as every image is downsampled by the same factor,
    //     relative sharpness ordering is preserved, which is what matters for
    //     "pick the sharpest of these 5 bursts".
    //
    // The 720 px cap is a tunable trade-off between focus-detection accuracy
    // (smaller cap hides fine-detail blur) and throughput. The current value
    // matches Lightroom's survey-mode thumbnail size class.
    //
    // For absolute sharpness scoring (not relative), a future improvement would
    // be to render a center-region crop at native resolution and combine it
    // with the downsampled metric. Tracked as a roadmap item.
