...
// Static regex patterns for XMP metadata editing. Compiled once, reused.
// Patterns are validated literals, so Lazy init never panics in practice.
static RE_RATING_ATTR: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"xmp:Rating\s*=\s*"[^"]*""#).expect("static rating-attr regex literal is valid")
});
static RE_RATING_TAG: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"<xmp:Rating\s*>[^<]*</xmp:Rating>"#)
        .expect("static rating-tag regex literal is valid")
});
static RE_LABEL_ATTR: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"xmp:Label\s*=\s*"[^"]*""#).expect("static label-attr regex literal is valid")
});
static RE_LABEL_TAG: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"<xmp:Label\s*>[^<]*</xmp:Label>"#).expect("static label-tag regex literal is valid")
});
static RE_LABEL_ATTR_WS: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"\s*xmp:Label\s*=\s*"[^"]*""#)
        .expect("static label-attr-ws regex literal is valid")
});
static RE_LABEL_TAG_WS: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"\s*<xmp:Label\s*>[^<]*</xmp:Label>"#)
        .expect("static label-tag-ws regex literal is valid")
});
static RE_LRTEMPLATE_XMP: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"(?s)s\.xmp = "(.*)""#)
        .expect("static lrtemplate-xmp regex literal is valid")
});
static RE_DC_SUBJECT: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"(?s)<dc:subject>\s*<rdf:Bag>.*?</rdf:Bag>\s*</dc:subject>"#)
        .expect("static dc-subject regex literal is valid")
});
...
pub fn duplicate_file(
    ...
    let mut source_rrexif_name = source_path.file_name().unwrap_or_else(|| std::ffi::OsStr::new("unknown")).to_os_string();
    source_rrexif_name.push(".rrexif");
    let source_rrexif = source_path.with_file_name(source_rrexif_name);

    if source_rrexif.exists() {
        let mut dest_rrexif_name = dest_path.file_name().unwrap_or_else(|| std::ffi::OsStr::new("unknown")).to_os_string();
...
pub fn copy_files(source_paths: Vec<String>, destination_folder: String) -> Result<(), String> {
    ...
    for source_image_path in unique_source_images {
        ...
        if source_parent == dest_path {
            ...
            let new_filename = new_base_path.file_name().unwrap_or_else(|| std::ffi::OsStr::new("unknown")).to_string_lossy();

            for original_file in all_files_to_copy {
                let original_full_filename = original_file.file_name().unwrap_or_else(|| std::ffi::OsStr::new("unknown")).to_string_lossy();
                let source_base_filename = source_image_path.file_name().unwrap_or_else(|| std::ffi::OsStr::new("unknown")).to_string_lossy();
...
pub fn move_files(
    ...
    for source_image_path in unique_source_images {
        ...
        let dest_image_path = dest_path.join(source_image_path.file_name().unwrap_or_else(|| std::ffi::OsStr::new("unknown")));
        renames.insert(
...
/// Seed a curated "国内场景" preset folder if it isn't already present.
///
/// This is idempotent: if a folder with the canonical name "国内场景" already
/// exists (regardless of its id), the command returns the current preset list
/// unchanged. The four presets cover the most common Chinese photography
/// workflows that RapidRAW's default preset library doesn't ship:
///
///   - 婚纱: bright, airy, slightly cool, lifted shadows for dress detail
///   - 儿童: warm, vibrant, gentle contrast for skin glow
///   - 证件照: neutral, high contrast, accurate colors per ISO/IEC 19794-5
///   - 旅拍: film-like, warm, faded blacks with subtle grain + vignette
///
/// Values are intentionally conservative — every numeric field is inside the
/// slider range validated by the frontend (exposure ±5, temperature ±100, etc.),
/// so applying a preset never clamps or distorts unexpectedly. Tone curve and
/// masks are not touched; only global tonal/color adjustments are set, leaving
/// the rest of the adjustment stack at image defaults.
#[tauri::command]
pub fn seed_default_presets(app_handle: AppHandle) -> Result<Vec<PresetItem>, String> {
    let mut current_presets = load_presets(app_handle.clone())?;

    const FOLDER_NAME: &str = "国内场景";

    let already_seeded = current_presets.iter().any(|item| match item {
        PresetItem::Folder(f) => f.name == FOLDER_NAME,
        _ => false,
    });
    if already_seeded {
        return Ok(current_presets);
    }

    // Wedding dress — bright, airy, slightly cool, soft contrast.
    // Lifted shadows preserve dress detail; negative clarity adds softness.
    let wedding = Preset {
        id: Uuid::new_v4().to_string(),
        name: "婚纱".to_string(),
    ...
    // Children — warm, vibrant, gentle contrast for natural skin glow.
    // Slight saturation boost with vibrance to avoid over-cooling skies.
    let children = Preset {
        id: Uuid::new_v4().to_string(),
        name: "儿童".to_string(),
        adjustments: serde_json::json!({
    ...
    // ID photo — neutral, high contrast, accurate colors.
    // Per ISO/IEC 19794-5: balanced exposure, no color cast, sharp.
    let id_photo = Preset {
        id: Uuid::new_v4().to_string(),
        name: "证件照".to_string(),
        adjustments: serde_json::json!({
        ...
        include_masks: Some(false),
        include_crop_transform: Some(false),
        preset_type: Some("technical".to_string()),
    };

    // Travel — film-like, warm, faded blacks, subtle grain + vignette.
    // Negative saturation + positive vibrance gives the muted-but-glowing
    // film look; warm temperature pushes shadows toward amber.
    let travel = Preset {
        id: Uuid::new_v4().to_string(),
        name: "旅拍".to_string(),
        adjustments: serde_json::json!({
    ...
    let folder = PresetFolder {
        ...
        children: vec![wedding, children, id_photo, travel],
    };

    // Place the curated folder first so it surfaces at the top of the presets
    // panel — these are the workflows most Chinese photographers reach for.
    current_presets.insert(0, PresetItem::Folder(folder));
    ...
    tauri::async_runtime::spawn_blocking(move || {
        for (i, source_path_str) in source_paths.iter().enumerate() {
            ...
            let import_result: Result<(), String> = (|| {
                ...
                if source_rrexif.exists() {
                    let mut dest_rrexif_name = dest_file_path.file_name().unwrap_or_else(|| std::ffi::OsStr::new("unknown")).to_os_string();
                    dest_rrexif_name.push(".rrexif");
...
pub fn rename_files(
    ...
    for (original_path, new_path) in &operations {
        let parent = original_path
            .parent()
            .ok_or("Could not get parent directory")?;
        let original_filename_str = original_path.file_name().unwrap_or_else(|| std::ffi::OsStr::new("unknown")).to_string_lossy();
        let new_filename_str = new_path.file_name().unwrap_or_else(|| std::ffi::OsStr::new("unknown")).to_string_lossy();
        ...
        if let Ok(entries) = fs::read_dir(parent) {
            for entry in entries.filter_map(Result::ok) {
                    ...
                    && entry_filename.ends_with(".rrdata")
                {
                    ...
                    let mut new_sidecar_name = new_path.file_name().unwrap_or_else(|| std::ffi::OsStr::new("unknown")).to_os_string();
        ...
        let mut old_rrexif_name = original_path.file_name().unwrap_or_else(|| std::ffi::OsStr::new("unknown")).to_os_string();
        old_rrexif_name.push(".rrexif");
        let old_rrexif = original_path.with_file_name(old_rrexif_name);

        if old_rrexif.exists() {
            let mut new_rrexif_name = new_path.file_name().unwrap_or_else(|| std::ffi::OsStr::new("unknown")).to_os_string();
 ...
 <rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
  <rdf:Description rdf:about=""
        ...
        && let Ok(mut content) = fs::read_to_string(&xmp_file)
    {
        ...
        let re_rating_attr = &*RE_RATING_ATTR;
        let re_rating_tag = &*RE_RATING_TAG;
        ...
        if let Some(lbl) = label {
            let re_label_attr = &*RE_LABEL_ATTR;
            let re_label_tag = &*RE_LABEL_TAG;
