...
pub fn run() {
    ...
    builder
        ...
        .setup(|app| {
            ...
            *state.lens_db.lock().unwrap_or_else(|e| e.into_inner()) = Some(Arc::new(lens_db));
            ...
            let main_window_cfg = app
                ...
                .find(|w| w.label == "main")
                .ok_or_else(|| {
                    "Main window config not found in tauri.conf.json".to_string()
                })?
                .clone();
            ...
            let mut window_builder =
                tauri::WebviewWindowBuilder::from_config(app.handle(), &main_window_cfg)
                    .map_err(|e| {
                        format!("Failed to create window builder from config: {e}")
                    })?;
            ...
            let window = window_builder.build().map_err(|e| {
                format!("Failed to build main window: {e}")
            })?;
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            android_integration::share_image_on_android,
            android_integration::pick_android_directory,
            android_integration::resolve_android_saf_pick,
            android_integration::list_android_saf_directory,
            ...
            mask_generation::generate_skin_mask,
            ...
            file_management::seed_default_presets,
