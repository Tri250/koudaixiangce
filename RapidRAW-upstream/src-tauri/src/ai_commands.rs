...
pub async fn generate_ai_depth_mask(
    ...
    let cached_depth = {
        ...
        let ai_state = ai_state_lock.as_mut().ok_or_else(|| "AI state not initialized".to_string())?;
...
pub async fn invoke_generative_replace_with_mask_def(
    ...
    {
        // M11: cloud endpoint is configurable. Resolution order:
        //   1. settings.cloud_base_url  (user-set override, persisted)
        //   2. RAPIDRAW_CLOUD_BASE_URL  (env var, e.g. CI / enterprise deploy)
        //   3. production default        (https://getrapidraw.com/api)
        // Empty string explicitly disables cloud and surfaces a clear error.
        let base_url = match settings.cloud_base_url.as_deref() {
            Some(s) if !s.trim().is_empty() => s.trim().to_string(),
            _ => std::env::var("RAPIDRAW_CLOUD_BASE_URL")
                .ok()
                .filter(|s| !s.trim().is_empty())
                .unwrap_or_else(|| "https://getrapidraw.com/api".to_string()),
        };
...
/// AI Color Match ("AI 追色"): sends the source image and a user-picked
/// reference image to a Qwen-VL (or any OpenAI-compatible vision) model,
/// which analyses the reference's color grading and returns a JSON contract
/// of adjustment parameters. The frontend merges these into the current
/// image's adjustments so the source takes on the reference's tonal style.
///
/// Configuration resolution order:
///   1. `vlm_api_key` / `vlm_endpoint` / `vlm_model` in app settings
///   2. `RAPIDRAW_VLM_API_KEY` env var (for the API key only)
///   3. DashScope defaults (endpoint + `qwen-vl-max-latest` model)
///
/// When no API key is configured the command surfaces a clear error string
/// instead of panicking, so the UI can guide the user to Settings → AI.
#[tauri::command]
pub async fn ai_color_match(
    source_path: String,
    reference_path: String,
    app_handle: tauri::AppHandle,
    state: tauri::State<'_, AppState>,
) -> Result<serde_json::Value, String> {
    // Touch `state` so the injected Tauri state is observably part of the
    // command contract even though this command does not need it directly;
    // keeping the parameter lets future revisions reuse cached image data.
    let _ = &state;

    // 1. Load settings.
    let settings = crate::app_settings::load_settings(app_handle.clone()).unwrap_or_default();

    // 2. Resolve VLM config (settings → env → error).
    let api_key = settings
        ...
        .or_else(|| {
            std::env::var("RAPIDRAW_VLM_API_KEY")
                .ok()
                .filter(|s| !s.trim().is_empty())
        })
        .ok_or_else(|| {
            "No VLM API key configured. Set vlm_api_key in settings or RAPIDRAW_VLM_API_KEY env var."
                .to_string()
        })?;
    ...
    // 3. Load both images off the async runtime (file I/O is blocking).
    ...
    let (source_image, reference_image) = tokio::task::spawn_blocking(move || {
        ...
        Ok::<_, String>((source_img, reference_img))
    })
    .await
    .map_err(|e| format!("Image loading task failed: {}", e))??;

    // 4. Call the VLM and return the validated adjustment contract.
    let adjustments = ai_connector::color_match_with_vlm(
        &source_image,
        &reference_image,
        &api_key,
        endpoint,
