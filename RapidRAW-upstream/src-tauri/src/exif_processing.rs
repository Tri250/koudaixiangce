...
pub fn write_image_with_metadata(
    ...
    // M13: TIFF metadata writing is intentionally a no-op.
    //
    // Background: kamadak-exif (the crate used here for reads) is read-only,
    // and the `image` crate's TIFF encoder does not preserve EXIF IFD tags.
    // Writing EXIF into TIFF without corrupting existing IFD0/offsets requires
    // either a dedicated writer crate (none are mature in the Rust ecosystem
    // as of 2026) or a fork of `tiff` with tag-passthrough support.
    //
    // Until such a crate is available, we conservatively skip metadata writing
    // for TIFF outputs and TIFF sources — silent loss of metadata is better
    // than corrupting the file. JPEG/PNG outputs still go through the normal
    // metadata preservation path below.
    //
    // Roadmap: track `image-rs/image` issue #1985 (TIFF EXIF write support)
    // and the `exifsd` / `exif-write` crates for a future implementation.
