...
class AiLlmColorGrader(private val context: Context) {
    ...
    private val prefs by lazy {
        // v2026.07: 使用 SafePreferences 包装，防止 XML 损坏导致 AI 配置丢失。
        SafePreferences.get(context, "ai_llm_color_grader", Context.MODE_PRIVATE)
    }
    ...
    suspend fun analyzeAndSuggest(
        ...
        try {
            ...
            // v2026.07: SafePreferences 防止清除 API Key 时崩溃。
            SafePreferences.remove(prefs, PREF_KEY_API_KEY)
            Result.failure(e)
    ...
    fun saveConfig(config: LlmConfig) {
        // v2026.07: SafePreferences 防止 AI 配置写入失败。
        SafePreferences.putString(prefs, PREF_KEY_PROVIDER, config.provider.name)
        SafePreferences.putString(prefs, PREF_KEY_MODEL, config.model)
        SafePreferences.putString(prefs, PREF_KEY_CUSTOM_ENDPOINT, config.customEndpoint)
        SafePreferences.putInt(prefs, PREF_KEY_MAX_TOKENS, config.maxTokens)
        SafePreferences.putFloat(prefs, PREF_KEY_TEMPERATURE, config.temperature)
        if (config.apiKey.isNotEmpty()) {
            SafePreferences.putString(prefs, PREF_KEY_API_KEY, encryptApiKey(config.apiKey))
        } else {
            SafePreferences.remove(prefs, PREF_KEY_API_KEY)
        }
    }
    ...
    fun loadConfig(): LlmConfig {
        val providerName = SafePreferences.getString(prefs, PREF_KEY_PROVIDER, null)
        val provider = try {
            providerName?.let { LlmProvider.valueOf(it) } ?: LlmProvider.ALIBABA_QWEN
        } catch (_: IllegalArgumentException) {
            LlmProvider.ALIBABA_QWEN
        }
        val encryptedKey = SafePreferences.getString(prefs, PREF_KEY_API_KEY, null)
        val apiKey = encryptedKey?.let { decryptApiKey(it) } ?: ""
        return LlmConfig(
            provider = provider,
            apiKey = apiKey,
            model = SafePreferences.getString(prefs, PREF_KEY_MODEL, "qwen-vl-plus") ?: "qwen-vl-plus",
            customEndpoint = SafePreferences.getString(prefs, PREF_KEY_CUSTOM_ENDPOINT, "") ?: "",
            maxTokens = SafePreferences.getInt(prefs, PREF_KEY_MAX_TOKENS, 1024),
            temperature = SafePreferences.getFloat(prefs, PREF_KEY_TEMPERATURE, 0.3f)
        )
    }
