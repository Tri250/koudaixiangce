...
object RawDecoder {
    ...
    /**
     * v1.7.0 hotfix: 懒加载 native 库，避免类加载时同步执行 System.loadLibrary。
     * 原 init{} 块在类首次引用时即触发加载，可能阻塞调用线程 10-50ms。
     * 改为显式调用 ensureNativeLibraryLoaded()，在真正需要解码时才加载。
     */
    @Synchronized
    private fun ensureNativeLibraryLoaded(): Boolean {
        if (loadAttempted) return nativeLibraryLoaded
        loadAttempted = true
        // v1.5.3 加固：
    ...
    fun isNativeAvailable(): Boolean = ensureNativeLibraryLoaded()
