...
class ImageProcessor {
    ...
    suspend fun processFullResolution(
        ...
        /** P2-D2.16: 原始 EXIF（用于读取 ISO 生成 NoiseProfile，启用 ISO 自适应降噪） */
        exif: ExifData? = null
        ...
        // P2-D2.16: 从 EXIF ISO 构建 RAW 噪点剖面，启用 ISO 自适应降噪
        val noiseProfile = exif?.iso?.toIntOrNull()?.let { NoiseProfile.fromIso(it) }
        val userWantsDenoise = n.lumaNoiseReduction > 1e-6f || n.colorNoiseReduction > 1e-6f
        // ISO 自适应：用户未指定强度时，若噪点等级 ≥ LOW 自动推荐降噪（跳过 MINIMAL 避免无谓开销）
        val autoDenoise = noiseProfile != null &&
            noiseProfile.level != NoiseProfile.NoiseLevel.MINIMAL
        if (userWantsDenoise || autoDenoise) {
        ...
        try {
            ...
            if (settings.format in exifFormats && settings.keepMetadata && originalExif != null) {
                try {
                    ...
                    originalExif.dateTime?.let { if (it.isNotEmpty()) {
                        exif.setAttribute(androidx.exifinterface.media.ExifInterface.TAG_DATETIME, it)
                        // P2-D2.8: 保留原始拍摄时间（DateTimeOriginal / Digitized）
                        // Lightroom/Photoshop 等读取 DateTimeOriginal 作为"拍摄时间"，
                        // 仅写 TAG_DATETIME 会导致导出后拍摄时间丢失/变为导出时间。
                        exif.setAttribute(androidx.exifinterface.media.ExifInterface.TAG_DATETIME_ORIGINAL, it)
                        exif.setAttribute(androidx.exifinterface.media.ExifInterface.TAG_DATETIME_DIGITIZED, it)
                    } }
