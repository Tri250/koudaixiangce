...
class EnhancedPresetManager(private val context: Context) {
    ...
    /**
     * 从文件导入 XMP / LrTemplate 预设。
     *
     * 自动检测格式，调用 [PresetConverter] 解析为 [Adjustments]，并保存为新的 STYLE 预设。
     *
     * @param uri 指向 .xmp / .lrtemplate 文件的内容 Uri
     * @return 转换结果（包含名称、调整参数与未映射字段警告）；读取或解析失败返回 null
     */
    suspend fun importPresetFromFile(uri: Uri): PresetConversionResult? =
        withContext(Dispatchers.IO) {
            runCatching {
                ...
                val fileName = uri.lastPathSegment ?: "preset"
                val result = PresetConverter.importAuto(content, fileName)

                val adjJson = json.encodeToString(result.adjustments)
                val preset = EnhancedPreset(
                    name = result.name.ifBlank { "Imported Preset" },
                    type = PresetType.STYLE,
                    adjustmentsJson = adjJson,
                    category = "imported",
                )
                savePreset(preset).getOrThrow()
                result
            }.getOrElse { e ->
                Log.e(TAG, "importPresetFromFile failed", e)
                null
            }
        }
