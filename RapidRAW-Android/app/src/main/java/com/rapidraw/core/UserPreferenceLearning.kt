...
class UserPreferenceLearning(context: Context) {
    ...
    private val prefs: SharedPreferences = SafePreferences.get(context, "rapidraw_prefs")
    ...
    fun recordAdjustment(scene: SceneType, adjustments: Adjustments) {
        ...
        val existing = SafePreferences.getStringSet(prefs, key, emptySet())?.toMutableSet() ?: mutableSetOf()
        existing.add(json.encodeToString(adjustments))
        SafePreferences.putStringSet(prefs, key, existing)
    }
    ...
    fun learn(): UserPreferenceProfile {
        ...
        scenes.forEach { scene ->
            val key = "edits_${scene.name.lowercase()}"
            val edits = SafePreferences.getStringSet(prefs, key, emptySet()) ?: emptySet()
        ...
        scenes.forEach { scene ->
            val key = "edits_${scene.name.lowercase()}"
            SafePreferences.getStringSet(prefs, key, emptySet())?.forEach { editJson ->
                try {
        ...
        val avgIntensity = if (totalEdits > 0) {
            scenes.flatMap { scene ->
                val key = "edits_${scene.name.lowercase()}"
                SafePreferences.getStringSet(prefs, key, emptySet())?.mapNotNull { editJson ->
                    try {
