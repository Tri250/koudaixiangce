...
class LensCorrector(
    ...
    companion object {
        ...
        /**
         * 从 Lensfun 数据库的镜头条目与插值参数创建 LensCorrector 实例。
         *
         * 将 lensfun 的 distortion(k1/k2/k3)、TCA(vr/vb)、vignetting(k1/k2/k3)
         * 插值结果转换为现有 Brown-Conrady + TCA 缩放 + 暗角多项式模型。
         * 各校正强度 0..1 用于用户微调（1.0 = 完全校正）。
         *
         * @param lens 已通过 [LensfunDatabase.findLens]/[LensfunDatabase.autodetectLens] 匹配到的镜头
         * @param focal 当前焦距（mm），用于在校准点间插值
         * @param aperture 当前光圈，用于暗角插值
         * @param distortionAmount 畸变校正强度 0..1
         * @param tcaAmount TCA 校正强度 0..1
         * @param vignetteAmount 暗角校正强度 0..1
         */
        fun fromLensfun(
            lens: LensfunDatabase.LensEntry,
            focal: Float,
            aperture: Float,
            distortionAmount: Float = 1f,
            tcaAmount: Float = 1f,
            vignetteAmount: Float = 1f,
        ): LensCorrector {
            val dist = LensfunDatabase.computeDistortionParams(lens, focal)
            val tca = LensfunDatabase.computeTcaParams(lens, focal)
            val vig = LensfunDatabase.computeVignettingParams(lens, focal, aperture, 10f)
            return LensCorrector(
                k1 = (dist?.k1 ?: 0f) * distortionAmount,
                k2 = (dist?.k2 ?: 0f) * distortionAmount,
                k3 = (dist?.k3 ?: 0f) * distortionAmount,
                p1 = 0f,  // lensfun 标准库不含切向畸变
                p2 = 0f,
                scale = 1f,
                tcaRedScale = 1f + ((tca?.vr ?: 1f) - 1f) * tcaAmount,
                tcaBlueScale = 1f + ((tca?.vb ?: 1f) - 1f) * tcaAmount,
                vignetteK1 = (vig?.k1 ?: 0f) * vignetteAmount,
                vignetteK2 = (vig?.k2 ?: 0f) * vignetteAmount,
                vignetteK3 = (vig?.k3 ?: 0f) * vignetteAmount,
            )
        }
