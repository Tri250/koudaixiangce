...
import com.rapidraw.core.EnhancedPresetManager.EnhancedPreset
...
class BatchProcessor(private val context: Context, private val imageProcessor: ImageProcessor) {
     ...
     * 批量应用预设到多张图片。
     *
     * P2-D2.11: 统一的预设批量应用入口。
     * 对每张图：
     * 1. 加载原图的当前 Adjustments（从 sidecar 或默认）
     * 2. 通过 EnhancedPresetManager.applyPreset 应用预设（支持强度插值）
     * 3. 委托给 batchApplyFilm 执行导出
     *
     * @param imageUris 图片 URI 列表
     * @param preset 要应用的预设
     * @param intensity 预设强度（0..1，null 表示使用预设默认强度）
     * @param exportSettings 导出设置
     * @param config 批处理配置
     * @return BatchProgress Flow
     */
    fun batchApplyPreset(
        ...
        val presetManager = EnhancedPresetManager(context)
        val defaultAdjustments = Adjustments()  // 默认调整作为基础
        val appliedAdjustments = presetManager.applyPreset(
            currentAdjustments = defaultAdjustments,
            preset = preset,
            intensity = intensity,
        ).getOrElse {
            // 预设应用失败，回退到默认 Adjustments（避免批量导出完全失败）
            Log.w(TAG, "applyPreset failed for '${preset.name}', falling back to default Adjustments: ${it.message}")
            defaultAdjustments
        }

        // 委托给 batchApplyFilm 执行批量导出
        batchApplyFilm(imageUris, appliedAdjustments, exportSettings, config).collect { progress ->
            emit(progress)
        }
    }
    ...
    /**
     * 批量应用预设（按预设名查找）。
     * 便利方法：通过预设名从 EnhancedPresetManager 查找后委托给 batchApplyPreset。
     *
     * @param imageUris 图片 URI 列表
    ...
    fun batchApplyPresetByName(
        ...
        val presetManager = EnhancedPresetManager(context)
        val preset = presetManager.presets.value.find { it.name == presetName }
        if (preset == null) {
            emit(BatchProgress(
                current = 0, total = 0,
                currentFileName = "",
        ...
        batchApplyPreset(imageUris, preset, intensity, exportSettings, config).collect { progress ->
            emit(progress)
        }
    }
