 ...
 * 读到同一份值，避免双源竞态。
 *
 * v2026.07 hotfix: 将 remember {} 改为 remember + mutableStateOf，
 * 确保在 Process Death & Restore 或引导完成后 navigate 回 Library 时，
 * NavHost 的 startDestination 可以被重新读取，避免因 remember 缓存导致
 * 引导完成后仍然进入 ONBOARDING 路由。
 */
@Composable
fun rememberStartDestination(): String {
    val context = LocalContext.current
    // 使用 mutableStateOf 确保值在 Compose 重组时能被重新读取，
    // 而非 remember {} 的惰性一次性求值。
    val isCompleted = remember { mutableStateOf(OnboardingState.isCompleted(context)) }
    return if (isCompleted.value) Routes.LIBRARY else Routes.ONBOARDING
}
