...
plugins {
    ...
    // ── Hilt 依赖注入框架 (v2.51.1) ───────────────────────────────
    // 迁移步骤 1: 添加 Hilt 插件，启用 KSP 处理 @HiltAndroidApp / @AndroidEntryPoint 注解
    // 注意：Hilt 使用 KSP 而非 KAPT，编译速度更快
...
dependencies {
    ...
    // ── Hilt 依赖注入框架 ───────────────────────────────────────────
    // 迁移步骤 2: 添加 Hilt 核心依赖 + KSP 编译器
    // 运行时依赖：提供 @Inject / @Singleton / @HiltViewModel 等注解支持
    implementation(libs.hilt.android)
    // KSP 编译器：生成依赖注入代码（替代 KAPT，编译更快）
    ksp(libs.hilt.compiler)
    // Hilt Navigation Compose 集成：支持 @HiltViewModel 在 Navigation 中自动创建
    // 使用 hiltViewModel() 替代 viewModel()，自动注入 SavedStateHandle 导航参数
