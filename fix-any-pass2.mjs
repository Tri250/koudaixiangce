...
// This script handles context-specific any replacements that the first pass couldn't handle
...
const SUB_MASK_CONFIG: Record<Mask, SubMaskConfigEntry> = {`
    ...
    // createSubMask(type, {} as unknown, mode) - already fixed
    
    // subMask.parameters as unknown -> need a different approach
    // The (subMask.parameters as unknown).XXX = ... patterns need casting
    // Since parameters is Record<string, ...>, we can use direct access
    ...
    // MaskContainerRow props - find the }: any) pattern
    content = content.replace(
    ...
    content = content.replace(
      ...
      'const SectionComponent: React.ComponentType<{ adjustments: MaskAdjustments; setAdjustments: (updater: MaskAdjustments | ((prev: MaskAdjustments) => MaskAdjustments)) => void; onDragStateChange?: (isDragging: boolean) => void }> | null = ({' 
    );
    // Fix the closing bracket
    // Actually this is too complex for simple replacement. Let me use a different approach.
    // Revert the above and use a simpler type
    content = content.replace(
      "const SectionComponent: React.ComponentType<{ adjustments: MaskAdjustments; setAdjustments: (updater: MaskAdjustments | ((prev: MaskAdjustments) => MaskAdjustments)) => void; onDragStateChange?: (isDragging: boolean) => void }> | null = ({",
    ...
    // t('editor.masks.params.' + param.key as unknown) - need to fix this
    content = content.replace(
  ...
  // ===== ImageCanvas.tsx specific fixes =====
  if (filePath.includes('ImageCanvas.tsx')) {
    // These are mostly useRef<any>, useState<any>, function params with : any
    // Let me handle them based on the line numbers from eslint output
  }
  
  // ===== Generic patterns across all files =====
  
  // (prev: any) => in setState calls - replace with proper context
  // This is tricky because we need to know the state type
  // For Adjustments state:
